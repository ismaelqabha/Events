...
const ProviderSetClientForBooking = props => {
      ...
      <View style={styles.offerTitle}>
        ...
        <View style={{marginRight: 20}}>
          {camp.priceInclude == 'perPerson' && (
            <View style={styles.includeView}>
              <Text style={styles.campText}>
                {'السعر للشخص ' + '(₪' + camp.campCost + ')'}
              </Text>
  ...
  const renderCampaighnSubHeader = camp => {
    return (
      <View>
        <TouchableOpacity
          style={styles.offerContentView}
          onPress={onOfferContentPress}>
  ...
  const renderCampaighnContentFromSub = camp => {
    return camp.contentFromSubDet.map(itemID => {
    ...
    return (
      <View style={styles.checkDataView}>
        <Text style={styles.text}>
          تنبية !! لقد تم اختيار عروض تحتوي على تفاصيل متشابة
        </Text>
      </View>
    );
  };
  const renderReservationDet = () => {
    ...
    return (
      <View style={{backgroundColor: bgColorDate}}>
        <Text style={styles.text}>
          قم بتحديد تفاصيل الحجز اِما من الخدمات او العروض
        </Text>
        <View
          style={{
            flexDirection: 'row',
          ...
          <TouchableOpacity
            style={
              pressed === 0 ? styles.detailLabelPressed : styles.detailLabel
            }
            onPress={() => {
          ...
          {isCampaign && (
            <TouchableOpacity
              style={
                pressed === 1 ? styles.detailLabelPressed : styles.detailLabel
              }
              onPress={() => {
    ...
    if (val === '') {
      ...
      if (!isNaN(intValue)) {
        ...
        setResDetail(prevState => {
          ...
          updatedResDetail[0] = {
            ...updatedResDetail[0],
            numOfInviters: intValue,
          }; // Update numOfInviters in resDetail
          return updatedResDetail;
        });
      }
    }
    ...
    return (
      <View>
        <View style={styles.dateView}>
          ...
          <Text style={styles.subDetText}>
            {moment(fulDate).format('dddd')}
          </Text>
        </View>
  ...
  return <View style={{marginTop: 30}}>{renderBiookingFields()}</View>;
};
