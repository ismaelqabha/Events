...
const ProviderSetPaymentForClient = ({
  ...
  const togglePaymentStatus = () => {
    setShowPaymentModal(true); // Open the modal when the button is clicked
  };
  ...
  const PaymentComponent = props => {
    ...
    const [paymentStatus, setPaymentStatus] = useState(
      props?.val?.paymentStutes || 'not paid',
    );
    ...
    const calculateAmountFromPersentage = pers => {
      ...
      const newPersentage =
        parseInt(adjustedPers)?.toFixed(1).toString() || '0';
    ...
    const calculatePersentageFromAmount = amou => {
      ...
      const currentTotalPercentage =
        checkSumPersentage() - parseFloat(paymentDataArray[index]?.pers || 0);
    ...
    const onChange = (event, selectedDate) => {
      ...
      if (paymentPolicy === 'pre' && chosenDate >= eventDate) {
        showMessage(
          'التاريخ يجب ان يكون قبل تاريخ الحدث نظرا لسياسة الدفع المسبق',
        );
        return;
      }
      // Set the valid date
        ...
        <Modal
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              ...
              {/* Conditionally show "Revert Payment" button */}
              {paymentStatus === 'paid' && (
                <TouchableOpacity
  ...
  const renderPaymentPolicyText = () => {
    switch (paymentPolicy) {
      ...
      case 'post':
        return null; // No text for post-payment
      default:
        return null;
    }
  };
  ...
  const onPaymentPress = async () => {
    ...
    const totalAmount = paymentDataArray.reduce(
    ...
    if (totalPercentage < 100 || totalAmount < totalPrice) {
      showMessage(
        'الدفعات غير مكتملة. تأكد من أن النسبة الإجمالية تصل إلى 100%.',
      );
      return;
    }
    ...
    try {
      ...
      const firstPaymentStatus =
        paymentDataArray[0]?.paymentStutes || 'not paid';
      const reqStatus =
        firstPaymentStatus === 'paid' ? 'partially paid' : 'waiting pay';
    ...
    return (
      <View style={styles.payMethodView}>
        <TouchableOpacity
          style={[
            styles.methodItem,
            cash ? styles.methodItemPress : styles.methodItem,
          ]}
          onPress={cashPress}>
        ...
        <TouchableOpacity
          style={[
            styles.methodItem,
            checks ? styles.methodItemPress : styles.methodItem,
          ]}
          onPress={checksPress}>
        ...
        <TouchableOpacity
          style={[
            styles.methodItem,
            creditCard ? styles.methodItemPress : styles.methodItem,
          ]}
